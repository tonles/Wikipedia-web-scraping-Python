...
def viewers_branch_2(soup, col, season_rows):
    ...
    vie_list_2 = []
    for i in range(0, 22):  # Take only the odd elements of the list "vie_list"
        if i % 2:
            continue
        else:
    ...
    vie_list_3 = []
    for i in range(21, 42):  # Take only the even elements of the list "vie_list"
        if i % 2:
            vie_list_3.append(vie_list[i])
        else:
            continue

    vie_list_4 = vie_list_2 + vie_list_3
    viewers_1_float = [float(i) for i in vie_list_4]
